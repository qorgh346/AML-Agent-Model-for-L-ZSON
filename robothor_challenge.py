...
from src.simulation.constants import THOR_LANDMARK_IDS
...
class RobothorChallenge:
    ...
    def __init__(self, cfg_file, agent_class, agent_kwargs, experiment_name, no_grad, env_type, class_type, render_depth=False, render_segmentation=False, class_remap=None, target_detector='glee-glip', object_detector='glip', seed=0):
    ...
    def inference_worker(
        ...
        while True:
            ...
            if class_remap is not None:
                ...
                agent.glee_module.remap_classes(class_remap[e["scene"]])
            ...
            while total_steps < max_steps and stopped is False:
                ...
                # plt.imshow(event.depth_frame, cmap='gray')
                ...
                if (object_detector == 'gt'
                        and 'renderInstanceSegmentation' in controller_kwargs and controller_kwargs['renderInstanceSegmentation']):
                    obj_bbox_info = {
                        "bbox": [],
                        "labels": [],
                        "scores": []
                    }
                    obj_list = list(event.instance_detections2D.instance_masks.keys())

                    for obj_id in obj_list:
                        if obj_id.split("|")[0] in THOR_LANDMARK_IDS:
                            obj_bbox_info["bbox"].append(list(event.instance_detections2D[obj_id]))
                            obj_bbox_info["labels"].append(obj_id.split("|")[0].lower())
                            obj_bbox_info["scores"].append(1.0)

                    obj_bbox_info["bbox"] = torch.tensor(obj_bbox_info["bbox"])
                    obj_bbox_info["scores"] = torch.tensor(obj_bbox_info["scores"])

                    observations["obj_bbox_info"] = obj_bbox_info

                action = None
                if no_grad:
                    with torch.no_grad():
                        action, log_dict = agent.act(observations, target_detector, object_detector)
                else:
                    action, log_dict = agent.act(observations, target_detector, object_detector)
                ...
                episode_metrics['custom_logs'].append(log_dict)
                stopped = action == "Stop"
            ...
            if not test:
                ...
                # target_visible = target_obj["visible"]
                target_visible = target_obj["distance"] < 1.65
                episode_metrics["success"] = stopped and target_visible
                print(f'Distance: {target_obj["distance"]}')
                print(f'STOP: {stopped} and VISIBLE: {target_visible}, so Success is {episode_metrics["success"]}')
    ...
    def inference(self, episodes, nprocesses=1, test=False):
        ...
        for worker_ind in range(nprocesses):
            ...
            thread_controller_kwargs['x_display'] = "0"  # f':0.{worker_ind % num_devices}'
