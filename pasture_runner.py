...
def main():
    ...
    parser.add_argument(
        "--target_detector",
        choices=['glee-glip', 'glee', 'glip', 'owl-vit', 'gt'],
        required=True,
        help="Target detector type.",
    )
    ...
    agent_type_to_explore_localize = {
        ...
        'src.models.agent_fbe_glee_glip': ['fbe', 'gleeglip'],
    ...
    for env_type, class_type in experiments:
        ...
        if args.target_detector == 'glee':
            threshold[1] = 0.69

        if isinstance(threshold, list):
            if args.model_path == '':
                agent_class, agent_kwargs, render_depth = agent.build(
                    fail_stop=fail_stop, prompts_path=args.template, threshold_glee=threshold[0],
                    threshold_glip=threshold[1], class_type=class_type,
                    env_type=env_type, clip_model_name=clip_model_name, center_only=args.center, class_remap=class_remap)
            else:
                with torch.no_grad():
                    if run_exploration_split:
                        agent_class, agent_kwargs, render_depth = agent.build(fail_stop=fail_stop,
                                                                              prompts_path=args.template,
                                                                              threshold_glee=threshold[0],
                                                                              threshold_glip=threshold[1],
                                                                              exploration_model_path=args.model_path,
                                                                              class_type=class_type, env_type=env_type,
            ...
            if args.model_path == '':
                agent_class, agent_kwargs, render_depth = agent.build(
                    fail_stop=fail_stop, prompts_path=args.template, threshold=threshold, class_type=class_type,
                    env_type=env_type, clip_model_name=clip_model_name, center_only=args.center)
            else:
                with torch.no_grad():
                    if run_exploration_split:
                        agent_class, agent_kwargs, render_depth = agent.build(fail_stop=fail_stop,
        ...
        import gzip
        with gzip.open(f'./{experiment_name}.json.gz', "wt", encoding="utf-8") as zipfile:
